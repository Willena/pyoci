...
def _mock_base_image():
    return ([], {"architecture": "amd64", "os": "linux", "config": {"Env": [], "WorkingDir": "/"}})
...
def test_oci_layout_structure():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        cfg = BuildConfig(
            tag="test:v1", output_dir=str(output), context_dir=str(ctx), use_cache=False
        )
...
def test_tag_extraction():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        output = Path(tmpdir) / "test-image"
        cfg = BuildConfig(
            tag="myapp:v2.1.0", output_dir=str(output), context_dir=str(ctx), use_cache=False
        )
        with patch("pyoci.builder.ImageBuilder._pull_base_image", return_value=_mock_base_image()):
            builder = ImageBuilder(cfg)
            builder.build()

        assert (output / "refs" / "tags" / "v2.1.0").exists(), (
            "Tag file not created with correct name"
        )

        cfg2 = BuildConfig(
            tag="latest", output_dir=str(output) + "2", context_dir=str(ctx), use_cache=False
        )
        with patch("pyoci.builder.ImageBuilder._pull_base_image", return_value=_mock_base_image()):
...
def test_output_dir_can_be_cleaned_before_build():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert not stale_file.exists(), (
            "Cleanup should delete stale files from the output directory"
        )
        assert (output / "index.json").exists(), (
            "Build output should still be recreated after cleanup"
        )
...
def test_build_emits_phase_logs(caplog):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with (
            ...
            patch("pyoci.builder.ImageBuilder._pull_base_image", return_value=_mock_base_image()),
        ):
            builder = ImageBuilder(cfg)
            builder.build()
...
def test_build_generates_requested_sbom_format():
    """Verify builder passes the configured SBOM format through."""
    with tempfile.TemporaryDirectory() as tmpdir:
        ctx = _create_test_context(tmpdir)
        output = Path(tmpdir) / "test-image"
        cfg = BuildConfig(
            ...
            output_dir=str(output),
            context_dir=str(ctx),
            generate_sbom="cyclonedx",
            use_cache=False,
        )

        with (
            patch("pyoci.builder.ImageBuilder._pull_base_image", return_value=_mock_base_image()),
            patch("pyoci.builder.generate_sbom") as mock_generate_sbom,
        ):
            builder = ImageBuilder(cfg)
            builder.build()

        mock_generate_sbom.assert_called_once_with(ctx, output / "sbom.json", format="cyclonedx")
