...
def test_docker_config_auth_provider():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        config = {
            "auths": {
                "ghcr.io": {"auth": "dGVzdHVzZXI6dGVzdHBhc3M="},
...
def test_chain_auth_provider():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        chain = ChainAuthProvider(
            [EnvironmentAuthProvider(), DockerConfigAuthProvider(config_path)]
        )
...
def test_registry_auth_from_user_pass_or_token():
    ...
    assert RegistryAuth.from_user_pass_or_token(
        password_or_token="token123"
    ) == RegistryAuth.bearer("token123")
    assert RegistryAuth.from_user_pass_or_token(
        username="user", password_or_token="pass"
