...
from pyoci.builder import ImageBuilder, parse_platform
from pyoci.config import BuildConfig
from pyoci.oci import build_config_json, build_index_json, OCILayer
...
def test_build_config_with_platform():
    ...
    result = build_config_json(
        "arm64", "linux", {"DEBUG": "true"}, "/app", ["python", "-m", "app"], [8080]
    )
...
def test_build_index_with_platform():
    ...
    index = build_index_json(
        "sha256:abc123", 1024, "myapp:v1", architecture="arm64", os_name="linux"
    )
...
def test_platform_manifest_selection():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("pyoci.registry_client.RegistryClient.pull_manifest") as mock_manifest:
            ...
            with patch(
                "pyoci.registry_client.RegistryClient.pull_blob", side_effect=mock_pull_blob
            ):
                with patch("pyoci.auth.get_auth_for_registry", return_value=None):
                    cfg = BuildConfig(
                        tag="test:v1",
                        base_image="python:3.11-slim",
                        context_dir=str(ctx),
