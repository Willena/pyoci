...
def test_parse_image_reference():
    ...
    assert parse_image_reference("docker.io/library/python:3.11") == (
        "docker.io",
        "library/python",
        "3.11",
    )
    assert parse_image_reference("localhost:5000/test:latest") == (
        "localhost:5000",
        "test",
        "latest",
    )
...
def test_registry_client_construction():
    ...
    client_basic = RegistryClient(
        "ghcr.io", "user/repo", auth=RegistryAuth.basic("alice", "secret")
    )
...
def test_push_blob_refreshes_bearer_token_after_pull_scoped_exists_check(tmp_path):
    ...
    def fake_urlopen(req):
        ...
        if (
            req.full_url
            == "https://auth.example/token?service=ghcr.io&scope=repository%3Auser%2Fapp%3Apull"
        ):
            return MockHTTPResponse(200, json.dumps({"token": "pull-token"}).encode())

        if (
            req.full_url
            == "https://auth.example/token?service=ghcr.io&scope=repository%3Auser%2Fapp%3Apull%2Cpush"
        ):
    ...
    assert (
        client._bearer_tokens[("https://auth.example/token", "ghcr.io", "repository:user/app:pull")]
        == "pull-token"
    )
    assert (
        client._bearer_tokens[
            ("https://auth.example/token", "ghcr.io", "repository:user/app:pull,push")
        ]
        == "push-token"
    )
...
def test_basic_auth_from_typed_auth_is_sent_on_first_request():
    ...
    client = RegistryClient(
        "registry.example.com", "team/app", auth=RegistryAuth.basic("user", "pass")
    )
    ...
    assert requests == [
        (
            "HEAD",
            "https://registry.example.com/v2/team/app/blobs/sha256:abc123",
            "Basic dXNlcjpwYXNz",
        )
    ]
