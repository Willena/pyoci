...
class RegistryClient:
    def __init__(self, registry: str, repository: str, auth: Optional[RegistryAuth] = None):
                    ...
                    if challenge:
                        ...
                        if token:
                            return self._make_request(
                                method, url, data, headers, retry_auth=False, auth_params=challenge
                            )
            body = b""
            try:
                body = e.read()
...
def parse_image_reference(ref: str) -> Tuple[str, str, str]:
    ...
    if "/" not in ref:
        return (
            "docker.io",
            f"library/{ref.split(':')[0]}",
            ref.split(":")[-1] if ":" in ref else "latest",
        )
