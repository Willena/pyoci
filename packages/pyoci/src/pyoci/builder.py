...
class ImageBuilder:
    ...
    def build(self):
        ...
        cfg = build_config_json(arch,os_name,self.config.env,self.config.workdir,entry,self.config.exposed_ports,
                                labels=self.config.labels,user=self.config.user,cmd=self.config.cmd,base_config=normalized_base_config,
    ...
    def _normalize_base_config(self, base_config: Optional[Dict], base_layers: List[OCILayer]) -> Optional[Dict]:
        ...
        normalized["rootfs"] = {
            "type": "layers",
            "diff_ids": base_diff_ids + [layer.digest for layer in base_layers[len(base_diff_ids):]],
        }
        return normalized
