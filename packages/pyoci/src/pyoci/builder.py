...
def parse_platform(platform: str) -> Tuple[str, str]:
    ...
    if len(parts) != 2:
        raise ValueError(
            f"Invalid platform format: {platform}. Expected 'os/arch' (e.g., 'linux/amd64')"
        )
...
class ImageBuilder:
    ...
    def build(self):
        ...
        if self.config.include_deps:
            ...
            if deps_layer:
                ...
                app_history.append({"created_by": "pyoci add dependency files"})

        app_layer = self._create_app_layer(layers_dir, include)
        app_layers.append(app_layer)
        app_diff_ids.append(app_layer.digest)
        app_history.append({"created_by": "pyoci add application files"})

        all_layers = base_layers + app_layers

        normalized_base_config = self._normalize_base_config(base_config, base_layers)
        ...
        index = build_index_json(
            manifest_digest, len(manifest_bytes), self.config.tag, arch, os_name
        )
        ...
        sbom_setting = getattr(self.config, "generate_sbom", False)
        if sbom_setting:
            sbom_path = output / "sbom.json"
            sbom_format = sbom_setting if isinstance(sbom_setting, str) else "spdx"
            logger.info("Generating SBOM...")
            generate_sbom(Path(self.config.context_dir), sbom_path, format=sbom_format)
    ...
    def _normalize_base_config(
        self, base_config: Optional[Dict], base_layers: List[OCILayer]
    ) -> Optional[Dict]:
        """Ensure the base config rootfs metadata describes every pulled base layer."""
        if not base_config:
            return base_config

        base_diff_ids = list(base_config.get("rootfs", {}).get("diff_ids", []))
        if len(base_diff_ids) == len(base_layers):
            return base_config

        normalized = deepcopy(base_config)
        normalized["rootfs"] = {
            "type": "layers",
            "diff_ids": base_diff_ids
            + [layer.digest for layer in base_layers[len(base_diff_ids) :]],
        }
        return normalized

    def _pull_base_image(
        self, layers_dir: Path, os_name: str, arch: str
    ) -> Tuple[List[OCILayer], Optional[Dict]]:
        ...
        for i, layer_desc in enumerate(manifest.get("layers", []), 1):
            ...
            if not layer_path.exists():
                logger.info(
                    "  Pulling layer %d/%d (%s...)", i, len(manifest["layers"]), layer_digest[:19]
                )
                client.pull_blob(layer_digest, layer_path)
            base_layers.append(
                OCILayer(layer_desc["mediaType"], layer_digest, layer_size, str(layer_path))
            )
    ...
    def _show_build_plan(self):
        ...
        if self.config.env:
            logger.info(
                "  Environment: %s", ", ".join(f"{k}={v}" for k, v in self.config.env.items())
            )
        if self.config.labels:
            logger.info(
                "  Labels: %s", ", ".join(f"{k}={v}" for k, v in self.config.labels.items())
            )
    ...
    def _create_app_layer(self, layers_dir, include_paths):
        ...
        files_sorted = (
            sorted(files, key=lambda x: x[1].as_posix()) if self.config.reproducible else files
        )
