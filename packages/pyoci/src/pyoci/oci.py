...
class OCIIndex:
    ...
    def to_json(self) -> Dict:
        ...
        if self.annotations:
            idx["annotations"] = self.annotations
        return idx
...
def build_config_json(
    ...
    if base_config:
        ...
        base_env = {
            ...
            for kv in base_config.get("config", {}).get("Env", [])
            if "=" in kv
        }
        merged_env = {**base_env, **env}
        cfg["config"]["Env"] = [f"{k}={v}" for k, v in merged_env.items()]
        cfg["config"]["WorkingDir"] = working_dir or base_config.get("config", {}).get(
            "WorkingDir", "/app"
        )
        ...
        if entrypoint:
            if (
                is_distroless(base_config)
                and entrypoint
                and entrypoint[0] in ["/bin/sh", "/bin/bash", "sh", "bash"]
            ):
        ...
        if cmd:
            cfg["config"]["Cmd"] = cmd
        elif "Cmd" in base_config.get("config", {}):
            cfg["config"]["Cmd"] = base_config["config"]["Cmd"]
        if user:
            cfg["config"]["User"] = user
        elif "User" in base_config.get("config", {}):
            cfg["config"]["User"] = base_config["config"]["User"]
        base_labels = base_config.get("config", {}).get("Labels", {})
        if labels:
            cfg["config"]["Labels"] = {**base_labels, **labels}
        elif base_labels:
            cfg["config"]["Labels"] = base_labels
...
def is_distroless(config: Dict) -> bool:
    ...
    return (
        "distroless" in labels.get("org.opencontainers.image.base.name", "").lower()
        or "distroless" in labels.get("name", "").lower()
    )
...
def build_index_json(
    manifest_digest, manifest_size, tag=None, architecture="amd64", os_name="linux"
):
    desc = OCIManifestDescriptor(
        "application/vnd.oci.image.manifest.v1+json",
        manifest_digest,
        manifest_size,
        {"architecture": architecture, "os": os_name},
    )
