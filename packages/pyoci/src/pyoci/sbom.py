...
def _generate_spdx(context_dir: Path) -> Dict:
    ...
    sbom = {
        ...
        "name": f"pyoci-{context_dir.name}",
        "documentNamespace": f"https://sbom.pyoci/{context_dir.name}/{_generate_doc_id()}",
        "creationInfo": {
...
def _get_python_packages(context_dir: Path) -> List[tuple]:
    ...
    try:
        result = subprocess.run(
            ["pip", "freeze"], capture_output=True, text=True, cwd=context_dir, timeout=10
        )
