...
A Hatch plugin that integrates [pyoci](https://github.com/willena/pyoci) to build OCI container images directly from your Hatch projects — no Docker required.
...
- 🔧 **Configuration via pyproject.toml** - Use `[tool.hatch.build.hooks.pyoci]` section
...
2. Create a container image with pyoci
...
# Container labels
[tool.hatch.build.hooks.pyoci.labels]
...
```toml
[tool.hatch.envs.default.build.hooks.pyoci]
tag = "myapp:dev"
base-image = "python:3.11"
push = false

[tool.hatch.envs.production.build.hooks.pyoci]
tag = "myapp:latest"
base-image = "python:3.11-slim"
push = true
sbom = "spdx"
...
[tool.hatch.envs.local.build.hooks.pyoci]
skip = true
```
...
jobs:
  build:
    ...
    steps:
      ...
      - name: Install Hatch
        run: pip install hatch hatch-oci
...
1. **Registers Build Hook** - Hatch calls the pyoci hook during build
2. **Reads Configuration** - Extracts settings from `pyproject.toml`
3. **Builds Container** - Uses pyoci to create OCI image
...
```toml
[tool.hatch.build.targets.wheel.hooks.pyoci]
tag = "myapp:wheel-build"

[tool.hatch.build.targets.sdist.hooks.pyoci]
...
```toml
[build-system]
requires = ["hatchling", "hatch-oci", "pyoci"]
```
...
| Feature | hatch-oci | Dockerfile | Buildpacks |
|---------|------------------|-----------|------------|
| No Docker daemon | ✅ | ❌ | ❌ |
| Pure Python | ✅ | ❌ | ❌ |
| Hatch native | ✅ | ❌ | ❌ |
...
- [pyoci](https://github.com/willena/pyoci) - Core container builder
- [poetry-oci](../poetry-oci/) - Poetry plugin
- [Hatch](https://hatch.pypa.io/) - Modern Python project manager

## Support

- [GitHub Issues](https://github.com/willena/pyoci/issues)
- [Documentation](https://github.com/willena/pyoci/tree/main/docs)
