...
A Poetry plugin that integrates [pyoci](https://github.com/willena/pyoci) to build OCI container images directly from your Poetry projects — no Docker required.
...
- 🔧 **Configuration via pyproject.toml** - Use `[tool.pyoci]` section
...
Or install pyoci and the plugin:

```bash
pip install poetry-oci pyoci
```
...
All configuration is under `[tool.pyoci]` in `pyproject.toml`:
...
2. **Applies Tool Configuration** - Merges `[tool.pyoci]` settings
...
jobs:
  build:
    ...
    steps:
      ...
      - name: Install plugin
        run: poetry self add poetry-oci
...
pyoci automatically detects and configures:
...
- Python 3.11+
- Poetry 1.2.0+
- pyoci (installed automatically as dependency)
...
- [pyoci](https://github.com/willena/pyoci) - The core container builder
- [hatch-oci](../hatch-oci/) - Hatch plugin for pyoci
