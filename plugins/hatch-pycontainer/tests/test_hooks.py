...
from hatch_pycontainer.hooks import ContainerBuildHook, hatch_register_build_hook
...
def fake_pycontainer(monkeypatch):
    package = types.ModuleType("pycontainer")
    package.__path__ = []

    config_module = types.ModuleType("pycontainer.config")
    config_module.BuildConfig = FakeBuildConfig

    builder_module = types.ModuleType("pycontainer.builder")
    builder_module.ImageBuilder = FakeImageBuilder

    monkeypatch.setitem(sys.modules, "pycontainer", package)
    monkeypatch.setitem(sys.modules, "pycontainer.config", config_module)
    monkeypatch.setitem(sys.modules, "pycontainer.builder", builder_module)
...
def test_dependencies_are_installed_in_build_env(tmp_path):
    ...
    assert hook.dependencies() == ["pycontainer-build"]
...
def test_initialize_reports_missing_dependency(tmp_path, monkeypatch):
    def fail_pycontainer_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name in {"pycontainer.config", "pycontainer.builder"}:
    ...
    with pytest.raises(RuntimeError, match="pycontainer-build is not installed"):
